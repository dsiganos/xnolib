...
def create_new_database(cursor: CMySQLCursor, name: str) -> None:
    cursor.execute("CREATE DATABASE %s" % name)
    cursor.execute("USE %s" % name)
    cursor.execute("SET SQL_SAFE_UPDATES = 0")


def create_db_structure_frontier_service(cursor: CMySQLCursor) -> None:
...
def query_accounts_different_hashes(cursor: CMySQLCursor) -> CMySQLCursor:
